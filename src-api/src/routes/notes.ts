...
import { eq, and, or, desc, like, lte, inArray, sql } from 'drizzle-orm';
 ...
 *
 * 复用方式：这段 CTE 既供 /notes/tags 做计数，也供 /notes?tag=xxx 反查笔记 id，
...
export default async function (app: FastifyInstance) {
  app.get('/notes', async (req) => {
    ...
    if (tag) {
      const ids = (sqlite.prepare(SQL_IDS_BY_TAG).all({ uid: CURRENT_USER, tag }) as { id: number }[]).map(
        (r) => r.id,
      );
    ...
    // —— 智慧筛选器 ——
    // mastery_lte：找出「掌握度还很低、该重点攻克」的笔记
    const masteryLte = q.mastery_lte ?? q.masteryLte;
    if (masteryLte !== undefined && masteryLte !== '') {
      const v = Number(masteryLte);
      if (Number.isFinite(v)) conds.push(lte(wbNote.mastery, v));
    }
    // has_summary=false：总结栏为空 = 康奈尔笔记没闭环，属于「半成品」待补
    const hasSummary = q.has_summary ?? q.hasSummary;
    if (hasSummary !== undefined && hasSummary !== '') {
      const want = String(hasSummary) === 'true' || String(hasSummary) === '1';
      conds.push(want ? sql`trim(${wbNote.summaryColumn}) <> ''` : sql`trim(coalesce(${wbNote.summaryColumn}, '')) = ''`);
    }
   ...
   */
  app.get('/notes/tags', async () => {
    const rows = sqlite.prepare(SQL_TAG_COUNTS).all({ uid: CURRENT_USER }) as {
      name: string;
      count: number;
    }[];
    return rows;
   ...
   */
  app.get('/notes/backlinks/:id', async (req, reply) => {
    const id = Number((req.params as any).id);
    const self = db.select().from(wbNote).where(eq(wbNote.id, id)).get() as any;
    if (!self) return reply.code(404).send({ message: 'not found' });
    ...
    const colMatches = (col: any) =>
      patterns.map((p) => sql`${col} LIKE ${p} ESCAPE '\\'`);
    ...
    const rows = db
      ...
      .where(
        and(
          ...
          or(
            ...colMatches(wbNote.noteColumn),
            ...colMatches(wbNote.cueColumn),
            ...colMatches(wbNote.summaryColumn),
          ),
        ),
      )
    ...
    return rows.map((r) => {
      ...
      const hit = [r.noteColumn, r.cueColumn, r.summaryColumn].find((c: string) =>
        (c || '').includes(`[[${title}`),
      );
   ...
   * 双链解析：[[标题]] → 笔记 id。
   *
   * 为什么不复用 /notes?keyword=：keyword 是三栏模糊搜索，
   * 「Vue」会同时命中正文里提到 Vue 的一堆笔记，点一下双链跳错地方比不跳更糟。
   * 这里走标题精确匹配（大小写不敏感），命中不了就返回 exists:false，
   * 前端据此把该双链渲染成「未创建」样式并提供「新建同名笔记」入口。
   */
  app.get('/notes/resolve', async (req) => {
    const raw = String((req.query as any).title ?? '').trim();
