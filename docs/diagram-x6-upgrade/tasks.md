...
| ID | 具体内容 | 验收标准 | 依赖 |
|---|---|---|---|
| P1-T5.1（0.5d） | **自由画笔节点迁移**：改造 `DrawingNode.vue` → `nodes/X6DrawingNode.vue`；**继续保留 overlay 拦截 pointer 方案**（防 Tauri WKWebView X6 pointer 吞事件不兼容），结束后把 path d 写成 X6 `Shape.Path`；节点上支持改色、移动、缩放、描边宽；**画完 1 条只入 1 条 history**（画笔过程中暂停 `history.freeze`） | ① 画笔模式下按住鼠标拖拽可绘连续自由曲线 ② 松手后整条曲线作为 1 个节点，可整体拖动 ③ Ctrl+Z 撤销整条曲线（不是一像素撤销） ④ 改颜色 picker → 曲线变色 | P1-T4.1 |
