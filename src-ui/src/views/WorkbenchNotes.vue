<template>
  <div class="space-y-4 animate-fade-in">
    ...
    <div v-else-if="list.length === 0" class="rounded-xl border p-8 text-center" style="background: var(--kb-card); border-color: var(--kb-border);">
      ...
      <button class="kb-btn kb-btn-primary mt-3" @click="router.push('/workbench/notes/new')"><Icon name="plus" :size="14" /> 立即创建</button>
    </div>
      ...
      >
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-1 ml-auto" @click.stop>
            ...
            <button class="wb-icon-btn" title="删除" @click="remove(n)"><Icon name="trash-2" :size="14" /></button>
          </div>
        </div>
      </div>
    </div>
