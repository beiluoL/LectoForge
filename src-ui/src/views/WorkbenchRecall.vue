    ...
    <section v-else class="recall-detail">
      ...
      <div v-if="activeSession.status !== 'COMPLETED'" class="recall-write">
        ...
        <div class="recall-write-actions">
          ...
          <button
            class="kb-btn ai-btn"
            :disabled="aiScoring || !currentText.trim()"
      ...
      <!-- AI 语义评分结果面板（仅作参考，绝不回写 SM-2 三轮得分） -->
      <div v-if="aiScore" class="ai-panel">
        ...
        <div class="ai-score-row">
          <span class="ai-score" :style="{ color: scoreColor(aiScore.score) }">{{ aiScore.score }}</span>
          <span class="ai-score-unit">/100</span>
          <span class="ai-score-compare">规则法字面命中 {{ aiScore.ruleScore }}</span>
        </div>
        <div class="ai-score-bar"><i :style="{ width: aiScore.score + '%' }"></i></div>

        <div v-if="aiScore.missedPoints.length">
          <p class="ai-subtitle">遗漏要点</p>
          <ul class="ai-list is-danger">
            <li v-for="(p, i) in aiScore.missedPoints" :key="'m' + i">{{ p }}</li>
          </ul>
        </div>
        <div v-if="aiScore.wrongPoints.length">
          <p class="ai-subtitle">记错内容</p>
          <ul class="ai-list is-muted">
            <li v-for="(p, i) in aiScore.wrongPoints" :key="'w' + i">{{ p }}</li>
          </ul>
        </div>
        <p v-if="aiScore.feedback" class="ai-feedback">{{ aiScore.feedback }}</p>
        <span class="ai-meta">{{ aiScore.model }} · {{ aiScore.latencyMs }}ms</span>
      </div>
      ...
      <div v-if="activeSession.scoreTrend && activeSession.scoreTrend.some((s) => s != null)" class="recall-trend">
        ...
        <div v-if="activeSession.status === 'COMPLETED'" class="recall-advice">
          ...
          <div v-if="aiAdvice" class="ai-panel">
            ...
            <div v-if="aiAdvice.strengths.length">
              <p class="ai-subtitle">做得好的</p>
              <ul class="ai-list">
                <li v-for="(s, i) in aiAdvice.strengths" :key="'st' + i">{{ s }}</li>
              </ul>
            </div>
            <div v-if="aiAdvice.gaps.length">
              <p class="ai-subtitle">还差的</p>
              <ul class="ai-list is-danger">
                <li v-for="(g, i) in aiAdvice.gaps" :key="'gp' + i">{{ g }}</li>
              </ul>
            </div>
            <div v-if="aiAdvice.advice.length">
              <p class="ai-subtitle">练习策略</p>
              <ul class="ai-list">
                <li v-for="(a, i) in aiAdvice.advice" :key="'av' + i">{{ a }}</li>
              </ul>
            </div>
            <div v-if="aiAdvice.nextSteps.length">
              <p class="ai-subtitle">下一步</p>
              <ul class="ai-list">
                <li v-for="(n, i) in aiAdvice.nextSteps" :key="'ns' + i">{{ n }}</li>
              </ul>
            </div>
          </div>
        </div>
...
// ===== AI 语义评分（D1）：仅作参考，不影响 SM-2 三轮得分的确定性 =====
const aiScore = ref<RecallScoreResult | null>(null)
const aiScoring = ref(false)
const aiHintVisible = ref(false)

// ===== AI 三轮改进建议（D2）：仅作建议展示，不改动任何记录 =====
const aiAdvice = ref<RecallAdviceResult | null>(null)
const aiAdvising = ref(false)
...
/**
 * 主动回忆「语义还原度」AI 评分（D1）。
 * 只读取当前书写内容做一次性语义比对，结果仅作参考展示，
 * 绝不回写 roundXScore（保持 SM-2 三轮流转与得分的确定性），
 * 未配置 AI 时降级为提示引导，不阻断任何主流程。
 */
async function runAiScore() {
  ...
  try {
    aiScore.value = await scoreRecallSemantic({
      sourceText: activeSession.value.sourceText,
      recallText: currentText.value,
      round: activeSession.value.currentRound,
    })
  } catch (e) {
...
/**
 * 三轮闭卷默写「趋势改进建议」（D2）。
 * 仅在会话已完成（三轮都有记录）时提供，读取三轮文本/分数交由 LLM 给策略，
 * 仅展示、不回写任何字段，未配置 AI 时降级为引导条。
 */
async function runAdvice() {
  if (!activeSession.value || activeSession.value.status !== 'COMPLETED') return
  aiAdvising.value = true
  aiHintVisible.value = false
  try {
    aiAdvice.value = await adviseRecall({ sessionId: activeSession.value.id })
    notify('已生成改进建议', 'success')
  } catch (e) {
    const msg = getApiError(e, 'AI 建议生成失败')
    if (msg.includes('AI 设置') || msg.includes('未配置') || msg.includes('已关闭')) {
      aiHintVisible.value = true
    }
    notify(msg, 'error')
  } finally {
    aiAdvising.value = false
  }
}
...
.recall-diff-text :deep(.rc-miss) {
  background: color-mix(in srgb, var(--kb-destructive) 18%, transparent);
...
.recall-imp-up { color: var(--kb-accent); }
.recall-imp-down { color: var(--kb-destructive); }
