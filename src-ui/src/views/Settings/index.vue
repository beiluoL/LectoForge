...
// 直达跳转：
// - ?section= 查询参数（托盘「快捷键设置」等外部入口）；
// - #hash 锚点（useSpeechToText 的 /settings#local-model）。
// 用户在设置页内时 query/hash 变化不会重挂载组件，故用 watch 响应（immediate 覆盖冷启动直达）。
watch(
  () => [route.query.section, route.hash] as const,
  ([sec, hash]) => {
    if (typeof sec === 'string' && SECTION_KEYS.includes(sec)) {
      activeKey.value = sec
      return
    }
    if (hash && hash.startsWith('#')) {
      const key = hash.slice(1)
      if (SECTION_KEYS.includes(key)) activeKey.value = key
    }
  },
  { immediate: true },
)
