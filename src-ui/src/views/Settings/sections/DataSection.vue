<template>
  <div>
    ...
    <section class="kb-set-card">
      <div class="kb-set-card-head">
        <Icon name="download" :size="18" class="kb-set-card-icon" />
        <div>
          <h3 class="kb-set-card-title">数据导出</h3>
          <p class="kb-set-card-desc">把学习数据导出为 CSV，随时可迁移或留档。</p>
        </div>
      </div>
      <div class="kb-row" style="gap:.5rem; flex-wrap: wrap;">
        <button class="kb-btn kb-btn-sm" @click="s.downloadCsv('reviews')">
          <Icon name="file-text" :size="14" /> 复习记录
        </button>
        <button class="kb-btn kb-btn-sm" @click="s.downloadCsv('habits')">
          <Icon name="calendar-check" :size="14" /> 习惯打卡
        </button>
        <button class="kb-btn kb-btn-sm" @click="s.downloadCsv('tasks')">
          <Icon name="list-checks" :size="14" /> 任务清单
        </button>
      </div>
    </section>
