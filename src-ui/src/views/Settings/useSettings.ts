 ...
 *
 * 三栏式布局（2026-08-26 恢复）：把设置页脚本逻辑整体下沉，供 index.vue 外壳与各
 * section 组件共享同一份状态。涵盖：数据目录、AI 模型服务、本地模型（LLM/Whisper/TTS）、
 * 数据备份、数据导出（CSV）、AI 能力清单，以及跨 section 的「未保存改动」守卫（dirty / snapshot / baseline）。
 *
 * ⚠️ 架构红线（不改动后端契约）：
 ...
 * - 外观偏好（主题/强调色/紧凑模式）不走本文件：由 app-store.settings.ui + utils/ui-prefs 统一管理
 *   （main 分支现行架构），AppearanceSection 直接读写 appStore.settings.ui 并调用 applyTheme/applyAccent/applyDensity。
 */
